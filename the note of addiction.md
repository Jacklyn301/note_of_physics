...
B --> C(["🐀 Impoverished<br/>「這裡除了我什麼都沒有」"])
B --> D(["🐀🐀🐀 Standard<br/>「正常上班族宿舍」"])
B --> E(["🐀🐀🐀🐀🐀🐀<br/>🐀🐀🐀🐀 Enriched<br/>「幹，今天又有新玩具？」"])

C --> F(["經驗少"])
D --> G(["Baseline"])
E --> H(["刺激多<br/>探索 + 社交 + 新奇物件"])
...
```mermaid
graph LR
A{158隻老鼠們<br>🐁🐁🐁}-->B{四個<br>選擇}-->P([A/B，不利選擇<br>多獎勵<br>但是time-out太長])-.->R1([現在爽，但長期虧])
B-->G([C/D，有利選擇<br>獎勵較少<br>但是time-out短])-.->R2([現在少拿一點，但長期賺])
```
...
```mermaid
flowchart LR
cortex{皮質}-->|麩胺酸+|S([紋狀體])-->|GABA-|GS([內蒼白球/黑質])-->|GABA-|Th([視丘])-->|麩胺酸+|cortex
```
...
```mermaid
flowchart LR
cortex{皮質}-->|麩胺酸+|S([紋狀體])-->|GABA-|G([外蒼白球])-->|GABA-|STN([視丘下核])-->GS([內蒼白球/黑質])-->|GABA-|Th([視丘])-->|麩胺酸+|cortex
```
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/patterns_of_time-dependent_gene_regulation_in_NAc_after_cocaine_self-administration_0906.png)
...
### 讓我們回到理論 😗
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/time-dependent_control_of_gene_regulation_by_drug_exposure_via_epigenetic_mechanisms_0906.png)
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/cell_type%E2%80%93specific_control_of_chromatin_accessibility_in_NAc_by_acute_and_chronic_cocaine_exposure_0906.png)
